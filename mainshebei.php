...
function qqdh(){
   global $db;
    ...
    $row2 = $db->prepare("select Device_id,AddressInfo,sid from wp_ischool_telephone where sid NOT in(10000,56623,56689,56707,56675,56748,56732) ORDER BY sid,Device_id");
    $row2->execute();
...
//                      exit();
                    ...
                    {
                       $new['zc'][$k]['pingan_zcid'] = $value["pinganid"];
                    }
                }
            }
        }
        ...
        foreach ($row as $k => $v) {
            foreach ($newrow as $key => $value) {
                ...
                if ($v[0] == $value['id']) {
                    ...
                    $new['qbzc'][$v[0]]['sid'] = $v[0];
                    $new['qbzc'][$v[0]]['sname'] = snameConfig($v[0]);
                    $new['qbzc'][$v[0]]['pingan_bzcid'] = $value["pinganid"];
                }
            }
        }
    }
    ...
    if (!empty($new['qbzc'])) {
        foreach ($new['qbzc'] as $key => $value) {
           ...
           foreach ($value['pingan_bzcid'] as $k => $v) {
               ...
               $pa_name = $v.'号机';
               $status = 1;
               addqqsb($sid,$school,$pa_id,$pa_name,$ctime,$status);
           }
        }
    }
    if (!empty($new['bfzc'])) {
...
function querysid(){
    global $db;
    $stmt = $db->prepare("select id,name,pinganid from wp_ischool_school where is_deleted = 0 and pinganid is not null and id not in(10000,56731,56683,10000,56675,56748,56732)");
    $stmt->execute();
...
function addqqsb($sid,$school,$pa_id,$pa_name,$ctime,$status){
    global $db;
    $stmt = $db->prepare("insert into wp_ischool_qqsb(sid,school,pa_id,pa_name,ctime,status) values(?,?,?,?,?,?)");
    $stmt->execute([$sid,$school,$pa_id,$pa_name,$ctime,$status]);
}
function sendmsgts($bfzc,$bzc){
    // oUMeDwHY58TN7eGHRMYabhEzOvAg 张豪openid  oUMeDwKrzFj4FePyVvHKzj9JFOig 杨茫 oUMeDwOm5E5q5enit-JM0L2JIgJE 谷战胜 oUMeDwEvvR3ZVPAT1toJ3LZj3QcM 王军民 oUMeDwFcvNn2P48APhcw8Bl32Zqc谷光輝 oUMeDwN9ovKs_1abf9tAbEv3hzdI 谷广亚 oUMeDwFE-5UKQjWSQ1kg-f9xfcAI 胡建辉 
    // $tos = ["oUMeDwLBklMzOqyGuxhuA-Pmzsu0","oUMeDwC5bsoGmgX6mC8qk3gzPnu8"];
    $tos = ["oUMeDwHY58TN7eGHRMYabhEzOvAg","oUMeDwEvvR3ZVPAT1toJ3LZj3QcM","oUMeDwN9ovKs_1abf9tAbEv3hzdI","oUMeDwOm5E5q5enit-JM0L2JIgJE","oUMeDwFE-5UKQjWSQ1kg-f9xfcAI","oUMeDwKrzFj4FePyVvHKzj9JFOig","oUMeDwFcvNn2P48APhcw8Bl32Zqc"];
